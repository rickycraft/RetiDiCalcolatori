...
public class StreamClient {
    public static void main(String[] args) {
        ...
        try {
            System.out.println("Servizi: C(occorrenza carattere)\t S(scarica file binari)\t ^D per terminare");
            
            while ((buffer = stdIn.readLine()) != null) {
                //operazione di Elimina
                if (buffer.equals("C")) {
                	outSock.writeUTF("C");
                	System.out.println("Inserisci il numero di occorrenze minimo");
                    ...
                    while(!continua) {
                    	...
                    	try {
                        	...
                        	System.out.println("DEVI INSERIRE CORRETTAMENTE IL NUMERO. Riprova\n");
                        	continua=false;
                        	buffer=stdIn.readLine();
                        }
                    }
                    outSock.writeInt(val);
                    continua=false;
                    System.out.println("Inserisci il carattere voluto\n");
                    buffer=stdIn.readLine();
                    while(!continua) {
                        if(buffer.length()!=1) {
                        	System.out.println("DEVI INSERIRE UN CARATTERE. Riprova\n");
                        	buffer=stdIn.readLine();            	
                        }else {
                        	continua=true;
                        }
                    ...
                    outSock.writeChar(buffer.charAt(0));
                    
                    //risposta dal server
                    int result=inSock.readInt();
                    if(result>=0) {
                    	System.out.println("Operazione eseguita con successo!\nHa trovato "+result+" righe con le occorrenze richieste\n");
                    }else {
