...
#pragma endregion
  /* CORPO DEL CLIENT: */
  printf("Inserisci il tipo e il luogo nell'ordine scritto separati da uno spazio: ");
  while (gets(tmp))
  {

    //controllo tmp
    char *copiatemp=malloc(strlen(tmp)+1);
    strcpy(copiatemp, tmp);
    char *tipo = strtok(copiatemp, " ");
    ...
    if (strcmp(tipo, "Concerto") == 0 || strcmp(tipo, "Calcio") == 0 || strcmp(tipo, "Formula1") == 0)
    {
      ...
      printf("Inserisci correttamente il tipo, puoi scegliere tra Concerto Calcio e Formula1\n");
    }
