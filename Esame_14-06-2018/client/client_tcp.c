...
#pragma region setup
  ...
  /* CORPO DEL CLIENT: */
  printf("Inserisci il direttorio remoto: ");
  while (gets(tmp))
  {

    while (tmp[0] != '/')
    {
      printf("Inserisci correttamente il direttorio\n");
      gets(tmp);
    }
...
char *length;
    ...
    {
      //parte info
      //faccio mandare NOMEFILE\nLUNGHEZZA\n
      
      if(c=='\n'){

        if(nome==NULL){
         nome=malloc(strlen(tmp)+1);
         strcpy(nome,tmp); 
        }
        else{
        length=malloc(strlen(tmp)+1);
        strcpy(length,tmp);
        }
